...
public class Controller_ris_categorico_pref {
    ...
    void premi_bottone(ActionEvent event) throws IOException {
    	if (bottone.getText().equals("Guarda voti candidati nel partito vincente")) {
    		//situazione in cui � presente un vincitore
    		FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/risultati_categorico_pref2.fxml"));
        	Parent root = loader.load();
        	
        	Controller_ris_categorico_pref2 contr = loader.getController();
        	contr.setVincitore(vincitore);
        	contr.inizializza(); //non posso usare initialize() perch� verrebbe eseguito prima del settaggio del vincitore
        	
        	Stage stage = (Stage)((Node)event.getSource()).getScene().getWindow();
    		Scene scene = new Scene(root);
    		stage.setScene(scene);
    		stage.setTitle("Elenco voti candidati nel partito vincente");
    		stage.show();
    ...
    void initialize() {
    	...
    	if (sessione.getTipo_vittoria().equals("maggioranza")) {
	    	if ((Integer)arr[arr.length - 1] == (Integer)arr[arr.length - 2]) {
	    		//situazione di pareggio tra due o pi� candidati
	    	...
	    	bottone.setText("Guarda voti candidati nel partito vincente");
    	} else { 
